...
qabar::qabar(TString name) {
  ...
  for(int isipm=0; isipm!=2; ++isipm) {
    amp_corr[isipm] =  new TH2D( Form("amp_corr_%d_%s",isipm,name.Data()), Form("amp_corr_%d;lowGain  mV;highGain  mV",isipm), 100, 0, 100, 100, 0, 1000 );
    amp_rati[isipm] =  new TH1D( Form("amp_rati_%d_%s",isipm,name.Data()), Form("amp_rati_%d;highGain  /  lowGain",isipm), 100, 0, 20.0);
...
void qabar::fill(bar *mybar) {
  for(int iSiPM=0; iSiPM!=2; ++iSiPM) {
    ...
    amp_corr[iSiPM]->Fill( -channelL->GetAmplitude(), -channelH->GetAmplitude() );
    Double_t rat = channelH->GetAmplitude() / channelL->GetAmplitude();
    if(!TMath::IsNaN(rat))
      amp_rati[iSiPM]->Fill( rat );
    int nclass=-1;
...
void qabar::saveas(TString file){
  ...
  for(int i=0; i!=2; ++i) {
    main->cd(1+i)->SetLogz(1);
    amp_corr[i]->DrawCopy("colz");
    main->cd(3+i);
    amp_rati[i]->DrawCopy();
  }
  main->SaveAs( Form("%s.pdf[",file.Data()), "PDF" );
  main->SaveAs( Form("%s_AmpCorr.root",file.Data()), "ROOT" );
